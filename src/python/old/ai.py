...
class AI:
	...
	def setup_policy(self, path_to_genetics):
		fin = open(path_to_genetics + self.name + ".ai", "rb")
		self.policies = pickle.load(fin)
		fin.close()
		print("Loading player " + self.name)

	def save(self, path_to_genetics):
		fout = open(path_to_genetics + self.name + ".ai", "wb")
